...
- `collection` (formerly `sub_tenant_id`) is a logical partition inside a tenant, typically user/workspace/team.
- Use the same `collection` on writes and reads. Data written under one sub-tenant should not be expected to appear from another.
...
| B2C app | one app/customer tenant; `collection = user_id` |
...
| `collection` | optional logical partition; default sub-tenant if omitted |
...
curl -s -X POST 'https://api.hydradb.com/tenants' \
  ...
  -d "{\"database\":\"${TENANT_ID}\"}"

# 2. Check readiness. Poll this until .data.infra.ready_for_ingestion is true.
curl -s "https://api.hydradb.com/tenants/status?database=${TENANT_ID}" \
  -H "Authorization: Bearer $HYDRA_DB_API_KEY" \
...
INGEST_ID=$(curl -s -X POST 'https://api.hydradb.com/context/ingest' \
  ...
  -H "API-Version: 2" \
  -F "type=memory" \
  -F "database=${TENANT_ID}" \
  -F "collection=${SUB_TENANT_ID}" \
...
curl -s -G 'https://api.hydradb.com/context/status' \
  -H "Authorization: Bearer $HYDRA_DB_API_KEY" \
  -H "API-Version: 2" \
  --data-urlencode "database=${TENANT_ID}" \
  --data-urlencode "collection=${SUB_TENANT_ID}" \
...
curl -s -X POST 'https://api.hydradb.com/query' \
  ...
  -H "API-Version: 2" \
  -H "Content-Type: application/json" \
  -d "{\"database\":\"${TENANT_ID}\",\"collection\":\"${SUB_TENANT_ID}\",\"query\":\"How should I answer Alex?\",\"type\":\"memory\",\"query_by\":\"hybrid\",\"mode\":\"thinking\"}" \
  | jq '.data.chunks'
```
