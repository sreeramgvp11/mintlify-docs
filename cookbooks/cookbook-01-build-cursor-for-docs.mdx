...
HEADERS = {
    "Authorization": f"Bearer {HYDRA_DB_API_KEY}",  # format is strict - Bearer + space + key
...
```python
import sys, os, json, time, subprocess, pathlib, requests
sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))
from config import BASE_URL, TENANT_ID, HEADERS
...
def upload_batch(batch: list) -> list:
    resp = requests.post(
        f"{BASE_URL}/ingestion/upload_knowledge",
        headers=HEADERS,
        json={"app_knowledge": json.dumps(batch), "tenant_id": TENANT_ID},
        timeout=30,
    )
    resp.raise_for_status()
...
├── config.py         - env vars, HYDRA_DB_API_KEY, HEADERS, OpenAI keys, recall defaults
...
- **`config.py`** - Loads `.env`, exposes `HYDRA_DB_API_KEY`, `TENANT_ID`, `HEADERS`, `OPENAI_API_KEY`, and recall defaults. Every other file imports from here only.
