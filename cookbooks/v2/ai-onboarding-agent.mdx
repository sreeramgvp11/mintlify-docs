...
| Sub-tenant isolation | Use `collection=f"hire-{hire_id}"` for per-hire memory. Company knowledge lives in the default sub-tenant. |
...
| Method | Purpose |
|---|---|
| `client.tenants.create(database=...)` | Create the onboarding tenant |
| `client.tenants.status(database=...)` | Check tenant is ready |
...
|---|---|---|
| `database` | `str` | Required. Your tenant ID. |
