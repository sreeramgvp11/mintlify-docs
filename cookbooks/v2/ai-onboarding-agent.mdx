...
```mermaid
graph LR
    A["Decision logs<br/>Org charts<br/>Meeting notes<br/>Product specs"] -->|"source.ingest()"| B["HydraDB<br/>tenant: onboarding<br/>sub_tenant: company-context"]
    C["New hire question"] -->|"query()"| B
    B -->|"retrieved context"| D["Onboarding agent"]
    D -->|"source.ingest(type=memory)"| E["HydraDB<br/>sub_tenant: hire-{id}"]
...
> **Batch limit**: Max 20 files per `source.ingest()` call. For large document sets, upload in batches with a 1-second sleep between them.
...
| `422 field required: memories` on `source.ingest(type="memory")` | Wrong field structure | Pass `memories=[{"text": "...", "infer": True}]` as a list of dicts. |
...
| Topic | Note |
|---|---|
| Batch size | Max 20 files per `source.ingest()` call. Sleep 1s between batches for large document sets. |
