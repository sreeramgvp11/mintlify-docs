...
def create_database():
    try:
        response = client.databases.create(database=DATABASE_ID)
        ...
        # Database may already exist if you've run this before.
        if "DATABASE_ALREADY_EXISTS" in str(exc):
    ...
    while True:
        status = client.databases.status(database=DATABASE_ID)
        ...
        print(
            f"  scheduler={infra.scheduler_status} "
            f"graph={infra.graph_status} "
            f"vectorstore={vectorstore} "
