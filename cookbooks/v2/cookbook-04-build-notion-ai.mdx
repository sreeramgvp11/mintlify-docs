...
resp = client.tenants.create(database=TENANT_ID)
print("Tenant:", resp)
```

> **Sub-tenants for teams:** Use `collection` to isolate data by department. Engineering, Sales, HR each get their own namespace within your tenant - no configuration needed, just pass the ID on upload.
...
**Endpoint:** `GET /context/status?ids=ID&database=TENANT` - Check indexing status
...
**GET** `/context/status?ids=ID&database=YOUR_TENANT` - Poll until status = "completed"
