...
import { FieldLegend } from "/snippets/field-legend.jsx";
...
| Field <FieldLegend show="green,amber,gray" /> | What it does | Details |
|---|---|---|
| `kind` <span className="field-req-amber">●</span> | Type of app object: `email`, `message`, `ticket`, `knowledge_base`, `comment`, `custom`. | [Identity fields](#identity-fields), [Kind-specific fields](#kind-specific-fields) |
| `provider` <span className="field-req-amber">●</span> | System the item came from, such as `slack`, `gmail`, `jira`, `notion`, or `salesforce`. | [Identity fields](#identity-fields) |
| `external_id` <span className="field-req-amber">●</span> | Stable provider ID for this exact item. | [Identity fields](#identity-fields) |
| `fields` <span className="field-req-green">●</span> | Structured content and app-specific fields for the selected `kind`. | [Kind-specific fields](#kind-specific-fields), [Thread and parent fields](#thread-and-parent-fields) |
| `metadata` <span className="field-req-gray">●</span> | Flexible context about where the item lives, such as channel, project, space, workspace, or pipeline. | [Container metadata](#container-metadata) |
| `relations` <span className="field-req-gray">●</span> | Explicit source-to-source connections, such as linked tickets, blockers, or CRM-to-support links. | [Relations](#relations) |
| `attachments` <span className="field-req-gray">●</span> | Files attached to this app item. Include `content.text` or `content.markdown` when you already extracted attachment text. | [Attachments and comments](#attachments-and-comments) |
| `comments` <span className="field-req-gray">●</span> | Embedded snapshot comments attached to this app item. For live incremental comments, prefer `kind: "comment"`. | [Attachments and comments](#attachments-and-comments), [Incremental ingestion](#incremental-ingestion) |
...
| Field <FieldLegend show="green,amber,gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `id` <span className="field-req-green">●</span> <br />(String) | Stable HydraDB source ID for this ingested item. | Used as the source ID returned by ingestion and for later updates/deletes. |
| `tenant_id` <span className="field-req-green">●</span> <br />(String) | Tenant that owns the item. | Routes the item into the correct tenant scope. |
| `sub_tenant_id` <span className="field-req-green">●</span> <br />(String) | Sub-tenant/user/workspace scope. Use `"default"` if you do not partition sources. | Routes the item into the correct sub-tenant scope. |
| `kind` <span className="field-req-amber">●</span> <br />(`email`, `message`, `ticket`, `knowledge_base`, `comment`, `custom`) | Tells HydraDB how to interpret `fields`. Defaults to `custom` if omitted. | Routes search by object type and selects the right parser for structured fields. |
| `provider` <span className="field-req-amber">●</span> <br />(`slack`, `gmail`, `jira`, `notion`, `salesforce`) | Names the app namespace the source came from. | Prevents ID collisions and lets queries scope to a provider, e.g. "Slack messages". |
| `external_id` <span className="field-req-amber">●</span> <br />(String) | Stable ID for this exact item in the provider. | Enables exact ID lookup and relation resolution across sources. |
| `fields` <span className="field-req-amber">●</span> <br />(Object) | Holds searchable content and structure. | Drives rendering, actors, thread/parent expansion, status/priority filtering, and relation derivation. |
| `metadata` <span className="field-req-gray">●</span> <br />(Object) | Flexible provider context such as channel, project, space, workspace, pipeline, region, or team. | Preserves source context for display and filtering without adding new typed fields for every app. |
| `relations` <span className="field-req-gray">●</span> <br />(Relation[]) | Explicitly connects this source to another source. | Enables connected-context retrieval for links, blockers, dependencies, and CRM/support relationships. |
| `attachments` <span className="field-req-gray">●</span> <br />(Attachment[]) | Adds files attached to the app item. | Makes attachment content searchable and can connect sources that share attachments. |
| `comments` <span className="field-req-gray">●</span> <br />(Comment[]) | Adds snapshot/backfill comments attached to the app item. | Makes embedded comment text and comment authors searchable. For incremental comments, use `kind: "comment"`. |

### `email` fields

| Field <FieldLegend show="green,amber,gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `kind` <span className="field-req-green">●</span> <br />(`email`) | Discriminator for email fields. | Validates and parses the email shape. |
| `subject` <span className="field-req-amber">●</span> <br />(String) | Email title/subject. | Improves title, snippets, and semantic matching. |
| `body` <span className="field-req-amber">●</span> <br />(String) | Main email content. | Primary searchable text. |
| `from` <span className="field-req-amber">●</span> <br />(String) | Sender. | Enables actor queries like "emails from Alice". |
| `to` <span className="field-req-gray">●</span> <br />(String[]) | Recipients. | Enables actor queries like "emails to the finance team". |
| `cc` <span className="field-req-gray">●</span> <br />(String[]) | CC recipients. | Adds actor context. |
| `bcc` <span className="field-req-gray">●</span> <br />(String[]) | BCC recipients when available and safe to expose. | Adds actor context. |
| `thread_id` <span className="field-req-amber">●</span> <br />(String) | Conversation/thread ID. | Enables same-thread expansion. |
| `reply_to_id` <span className="field-req-gray">●</span> <br />(String) | External ID of the email this replies to. | Connects replies to parent emails. |
| `forwarded_from_id` <span className="field-req-gray">●</span> <br />(String) | External ID of the original forwarded email. | Connects forwards to originals. |
| `created_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Email creation/sent time. | Helps chronology and recency-aware results. |
| `url` <span className="field-req-gray">●</span> <br />(String) | Link back to the email. | Improves citations and source navigation. |

### `message` fields

| Field <FieldLegend show="green,amber,gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `kind` <span className="field-req-green">●</span> <br />(`message`) | Discriminator for message fields. | Validates and parses the message shape. |
| `body` <span className="field-req-amber">●</span> <br />(String) | Message text. | Primary searchable text. |
| `author` <span className="field-req-amber">●</span> <br />(String) | Message sender. | Enables actor queries like "messages from Alice". |
| `thread_id` <span className="field-req-amber">●</span> <br />(String) | Shared thread/conversation ID. | Enables same-thread expansion. |
| `parent_id` <span className="field-req-gray">●</span> <br />(String) | External ID of the parent/replied-to message. | Connects replies to parent messages. |
| `mentions` <span className="field-req-gray">●</span> <br />(String[]) | People or handles mentioned in the message. | Enables actor queries like "messages mentioning Bob". |
| `created_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Message timestamp. | Helps chronology and recency-aware results. |
| `url` <span className="field-req-gray">●</span> <br />(String) | Link back to the message. | Improves citations and source navigation. |

### `ticket` fields

| Field <FieldLegend show="green,amber,gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `kind` <span className="field-req-green">●</span> <br />(`ticket`) | Discriminator for ticket fields. | Validates and parses the ticket shape. |
| `title` <span className="field-req-amber">●</span> <br />(String) | Ticket title. | Improves title, snippets, and semantic matching. |
| `description` <span className="field-req-amber">●</span> <br />(String) | Main ticket body. | Primary searchable text. |
| `status` <span className="field-req-amber">●</span> <br />(String) | Workflow state such as `open`, `closed`, `in_progress`. | Enables queries like "open tickets" or "closed issues". |
| `priority` <span className="field-req-gray">●</span> <br />(String) | Priority such as `high`, `p0`, `urgent`. | Enables priority-aware queries. |
| `assignee` <span className="field-req-gray">●</span> <br />(String) | Owner/assignee. | Enables actor queries like "tickets assigned to Alice". |
| `reporter` <span className="field-req-gray">●</span> <br />(String) | Creator/reporter. | Enables actor queries like "tickets reported by Bob". |
| `parent_id` <span className="field-req-gray">●</span> <br />(String) | Parent ticket/epic external ID. | Enables parent/child traversal. |
| `linked_issue_ids` <span className="field-req-gray">●</span> <br />(String[]) | Related ticket/issue external IDs. | Creates linked-ticket context for retrieval. |
| `created_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Ticket creation time. | Helps chronology and recency-aware results. |
| `updated_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Last update time. | Helps freshness and recency-aware results. |
| `url` <span className="field-req-gray">●</span> <br />(String) | Link back to the ticket. | Improves citations and source navigation. |

### `knowledge_base` fields

| Field <FieldLegend show="green,amber,gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `kind` <span className="field-req-green">●</span> <br />(`knowledge_base`) | Discriminator for knowledge-base fields. | Validates and parses the page shape. |
| `title` <span className="field-req-amber">●</span> <br />(String) | Page title. | Improves title, snippets, and semantic matching. |
| `body` <span className="field-req-amber">●</span> <br />(String) | Main page content. | Primary searchable text. |
| `parent_id` <span className="field-req-gray">●</span> <br />(String) | Parent page external ID. | Enables page hierarchy traversal. |
| `created_by` <span className="field-req-gray">●</span> <br />(String) | Page creator. | Enables actor queries like "docs created by Alice". |
| `updated_by` <span className="field-req-gray">●</span> <br />(String) | Last editor/updater. | Enables actor queries like "docs updated by Bob". |
| `created_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Page creation time. | Helps chronology and recency-aware results. |
| `updated_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Last update time. | Helps freshness and recency-aware results. |
| `url` <span className="field-req-gray">●</span> <br />(String) | Link back to the page. | Improves citations and source navigation. |

### `comment` fields

| Field <FieldLegend show="green,amber,gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `kind` <span className="field-req-green">●</span> <br />(`comment`) | Discriminator for first-class comment fields. | Validates and parses the comment shape. |
| `body` <span className="field-req-amber">●</span> <br />(String) | Main comment text. | Primary searchable text. |
| `author` <span className="field-req-amber">●</span> <br />(String) | Comment author. | Enables actor queries like "comments from Carol". |
| `parent_id` <span className="field-req-amber">●</span> <br />(String) | Parent source external ID in the same provider namespace. | Derives a `comment_on` relation to the parent source. |
| `thread_id` <span className="field-req-gray">●</span> <br />(String) | Shared discussion/activity thread ID. | Enables same-thread expansion around comments. |
| `created_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Comment creation time. | Helps chronology and recency-aware results. |
| `updated_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Comment edit/update time. | Helps freshness and recency-aware results. |
| `url` <span className="field-req-gray">●</span> <br />(String) | Link back to the comment. | Improves citations and source navigation. |
| `properties` <span className="field-req-gray">●</span> <br />(Object) | Extra comment metadata. | Carries provider-specific comment context. |

### `custom` fields

| Field <FieldLegend show="green,amber,gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `kind` <span className="field-req-green">●</span> <br />(`custom`) | Discriminator for custom fields. | Validates and parses the custom shape. |
| `parent_id` <span className="field-req-gray">●</span> <br />(String) | Parent `external_id` for custom hierarchies in the same `provider` namespace, e.g. account -> deal. | Enables parent/child traversal. |
| `thread_id` <span className="field-req-gray">●</span> <br />(String) | Shared activity stream or conversation ID. | Enables same-thread/activity-stream expansion. |
| `data` <span className="field-req-amber">●</span> <br />(Object) | App-specific structured payload. | Preserves CRM/business fields without requiring a new kind. |

### Attachment fields

| Field <FieldLegend show="gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `id` <span className="field-req-gray">●</span> <br />(String) | Stable attachment ID. | Helps deduplicate and connect attachment context. |
| `url` <span className="field-req-gray">●</span> <br />(String) | Attachment URL. | Lets results point back to the attachment. |
| `title` <span className="field-req-gray">●</span> <br />(String) | File name/title. | Improves attachment search and display. |
| `content_type` <span className="field-req-gray">●</span> <br />(String) | MIME type such as `application/pdf` or `image/png`. | Helps classify attachment content. |
| `size_bytes` <span className="field-req-gray">●</span> <br />(Integer) | Attachment size. | Useful for display/debugging. |
| `content.text` <span className="field-req-gray">●</span> <br />(String) | Pre-extracted plain-text attachment body. | Indexed in attachment-specific vectors. |
| `content.markdown` <span className="field-req-gray">●</span> <br />(String) | Pre-extracted Markdown attachment body. | Indexed like `content.text` when plain text is not provided. |
| `content.html_base64` <span className="field-req-gray">●</span> <br />(String) | Base64-encoded HTML, kept for structured attachment payloads. | Preserved in the payload; provide `content.text` or `content.markdown` for search. |
| `content.csv_base64` <span className="field-req-gray">●</span> <br />(String) | Base64-encoded CSV, kept for structured attachment payloads. | Preserved in the payload; provide `content.text` or `content.markdown` for search. |
| `properties` <span className="field-req-gray">●</span> <br />(Object) | Extra attachment metadata. | Carries provider-specific attachment context in the payload; use `title`, `url`, and `content` for searchable attachment context. |

### Embedded `comments[]` fields

| Field <FieldLegend show="amber,gray" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `id` <span className="field-req-gray">●</span> <br />(String) | Stable comment ID. | Helps deduplicate comments. |
| `author` <span className="field-req-amber">●</span> <br />(String) | Comment author. | Enables queries like "comments from Carol". |
| `body` <span className="field-req-amber">●</span> <br />(String) | Comment text. | Makes comments searchable. |
| `created_at` <span className="field-req-gray">●</span> <br />(ISO-8601 datetime) | Comment timestamp. | Helps chronology and recency-aware results. |
| `properties` <span className="field-req-gray">●</span> <br />(Object) | Extra comment metadata. | Carries provider-specific comment context in the payload; use `author` and `body` for searchable comment context. |

### Relation fields

| Field <FieldLegend show="gray,orange,blue" /> | Why send it | Search / ingestion effect |
|---|---|---|
| `predicate` <span className="field-req-gray">●</span> <br />(`linked_to`, `blocks`, `child_of`, `reply_to`, `comment_on`, `related_to`) | Names the relationship. | Lets search traverse only the relevant relationship type. |
| `target.source_id` <span className="field-req-orange">●</span> <br />(String) | Use when you already know the HydraDB source ID. | Connects directly to an existing source. |
| `target.external_id` <span className="field-req-orange">●</span> <br />(String) | Use when you know the provider ID but not the HydraDB source ID. | Lets HydraDB resolve or connect to the target by provider ID. |
| `target.provider` <span className="field-req-blue">●</span> <br />(String) | Disambiguates provider ID namespaces. | Prevents false links across apps that share the same ID text. |
| `properties` <span className="field-req-gray">●</span> <br />(Object) | Extra relation metadata, such as reason or confidence. | Preserves why the relation exists. |
