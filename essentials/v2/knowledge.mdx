...
Attach `tenant_metadata` at ingestion to enable deterministic filtering at recall time:
...
`tenant_metadata` keys must be defined in the tenant metadata schema with `enable_match: true` to be filterable. `document_metadata` is stored alongside the source for display and bookkeeping; to filter on it, nest the keys under `document_metadata` in `metadata_filters` (e.g. `{"metadata_filters": {"document_metadata": {"author": "Alice"}}}`).
