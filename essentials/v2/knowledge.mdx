...
| **Scope** | Shared across all users in a tenant | Per-user, scoped by `collection` (formerly `sub_tenant_id`) |
...
| <Field name="database" type="string" recommended /> | Tenant that owns the item. Must match form-level `database` (formerly `tenant_id`) when present. |
...
| <Field name="collection" type="string" /> | none | Scope this knowledge to a sub-tenant. If omitted, knowledge is available to all sub-tenants. |
