...
| Query over apps | `"knowledge"` | `"hybrid"` | `"thinking"` | Set `search_apps: true` to enable the app-aware retrieval lane (IDs, actors, thread & parent traversal) while still querying the full selected knowledge scope |
