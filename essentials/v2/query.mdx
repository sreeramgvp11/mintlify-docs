...
| --- | --- | --- |
| `sub_tenant_ids` | `string[]` or `{ [sub_tenant_id]: positive number (max one decimal place) }` | Preferred query-time scope selector. Use a single-item list for one user/workspace, a longer list to fan out with equal normalized weights, or an object to provide relative ranking weights with at most one decimal place. Maximum 100 sub-tenants. |
...
If you need to query several users, teams, or workspaces at once, pass `sub_tenant_ids`. A list gives every scope equal normalized weight; an object applies relative ranking weights with at most one decimal place before the final merged ranking. When `max_results` is set, it caps the final merged response across all selected sub-tenants:
...
**Cache the prompt context.** If the same query repeats inside a session, cache the `RetrievalResult` keyed by `(tenant_id, sub_tenant_ids, query, type, query_by)`. Normalize `sub_tenant_ids` before hashing - sort list values; for weighted objects, sort keys and keep weights at the documented one-decimal precision. Recall is fast, but skipping it entirely is faster.
