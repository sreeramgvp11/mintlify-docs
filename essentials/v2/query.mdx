...
If you need to query several users, teams, or workspaces at once, pass `sub_tenant_ids`. A list gives every scope equal normalized weight; an object applies relative ranking weights with at most one decimal place before the final merged ranking:
