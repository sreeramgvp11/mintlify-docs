...
| **Sent in** | `app_knowledge` as a stringified JSON object or array | `files` plus optional `file_metadata` |
...
| **Filtering / bookkeeping** | `tenant_metadata`, `additional_metadata` | Deterministic scoping and free-form display metadata. `metadata` aliases `tenant_metadata`; `document_metadata` aliases `additional_metadata`. |
...
The call returns `202 Accepted`; the source is not queryable until [`GET /source/status`](/api-reference/v2/endpoint/source-status) reports `indexing_status: "graph_creation"` or `"completed"`.
...
The ticket body lives in `fields.description`. Workflow fields belong in `fields`; scope/filter values like project and environment belong in `tenant_metadata`.
...
|---|---|
| `{ "source_id": "..." }` | You already know the HydraDB source ID. |
...
<Note>
  For app sources, prefer the app-native `relations[]` shape above. `relations.source_ids` is the file/legacy forceful-relation shortcut and does not carry predicate or provider information.
</Note>
...
For v2 Knowledge ingestion, `tenant_metadata` is the canonical filterable metadata field and `additional_metadata` is the canonical free-form metadata field.

| Use `tenant_metadata` when | Use `additional_metadata` when |
|---|---|
...
Filterable tenant metadata keys must be declared in the tenant schema. Query-time filters use top-level keys for `tenant_metadata` and nested `additional_metadata` for free-form fields:
...
| <Field name="tenant_metadata" type="object" /> | Schema-aligned filterable fields. Alias: `metadata`. | Queryable with top-level `metadata_filters` keys when declared in tenant schema. |
| <Field name="additional_metadata" type="object" /> | Free-form display/bookkeeping metadata. Alias: `document_metadata`. | Queryable under `metadata_filters.additional_metadata`. |
...
| <Field name="target.source_id" type="string" /> | Direct HydraDB source ID target. | Connects to a known source. |
...
| Using container IDs as `external_id` | Relations and exact lookup point to the channel/project/space instead of the item | Use the individual item ID as `external_id`; put channel/project/space in `tenant_metadata`. |
...
| Putting filterable fields only in `additional_metadata` | Filters are slower and require nested filter syntax | Put hot filters in `tenant_metadata` and declare them in the tenant schema. |
