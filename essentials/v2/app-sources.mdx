...
Send `app_knowledge` as a **JSON-stringified** form value. Each item should include the request-level tenant fields inside the item too; if supplied, they must match the form-level `database` and `collection`.
...
| <Field name="database" type="string" required /> | Tenant that owns the item. | Must match the form-level `database` when present. |
