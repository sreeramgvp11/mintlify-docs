...
| <Field name="database" type="string" required /> |  -  | Target tenant. The `database` field was formerly `tenant_id`. |
| <Field name="collection" type="string" recommended /> | default sub-tenant | User, workspace, or session scope for the memory. The `collection` field was formerly `sub_tenant_id`. |
...
| Omitting `collection` for per-user data | The memory lands in the default sub-tenant and may surface in the wrong scope | Use the user's ID as `collection` for B2C separation |
