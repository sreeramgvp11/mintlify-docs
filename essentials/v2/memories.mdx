...
When you call [`POST /source/ingest`](/api-reference/v2/endpoint/ingest-content) with `type=memory`, HydraDB turns your input into queryable user context.
...
| <Field name="title" type="string" /> | Short label for display in `/source/list` and query results. |
...
| <Field name="tenant_metadata" type="string (JSON object)" /> | Tenant-schema fields for filtering and query. For memory ingestion, encode this nested object as a JSON string inside each `memories[]` item, for example `"tenant_metadata": "{\"department\":\"support\"}"`. |
...
| Storing shared docs as memories | They will not surface when querying `type: "knowledge"` | Use [knowledge ingestion](/essentials/v2/knowledge) with `type=knowledge` on [`POST /source/ingest`](/api-reference/v2/endpoint/ingest-content) |
