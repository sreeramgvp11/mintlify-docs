...
| Not including `collection` (formerly `sub_tenant_id`) when querying memories | Queries the default sub-tenant instead of the user's | Always pass the same `collection` used at ingestion. |
