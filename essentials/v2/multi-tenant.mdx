...
<Note>
  The request fields are now named **`database`** (formerly `tenant_id`) and **`collection`** (formerly `sub_tenant_id`). The old names remain fully supported as deprecated aliases, so existing integrations keep working without any changes -- both spellings are accepted interchangeably.
</Note>
