...
curl -X POST 'https://api.hydradb.com/ingestion/upload_knowledge' \
  ...
  -F "sub_tenant_id=engineering_team_alpha" \
  -F "files=@engineering_standards.pdf" \
  -F 'file_metadata=[{"additional_metadata":{"title":"Engineering Standards","department":"Engineering"}}]'
```
