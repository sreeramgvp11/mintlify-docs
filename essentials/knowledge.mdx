...
Set the `relations` field on any item in `file_metadata` or `app_sources`:

```json
{
  "relations": {
    "hydradb_source_ids": ["source-id-1", "source-id-2"]
  }
}
```
  ...
  -F 'app_sources=[
    {
      ...
      "text": "...",
      "relations": { "hydradb_source_ids": ["auth-troubleshooting", "auth-faq"] }
    }
  ]'
```
```typescript TypeScript SDK
