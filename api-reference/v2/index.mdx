...
- **Tenant scoping.** Most tenant-scoped endpoints require a `database` (formerly `tenant_id`). Many source and query endpoints also accept an optional `collection` (formerly `sub_tenant_id`) for finer-grained scoping. If omitted, the default sub-tenant is used. The old `tenant_id`/`sub_tenant_id` names remain accepted as deprecated aliases.
