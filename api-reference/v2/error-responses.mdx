...
Tenant creation is asynchronous. After `POST /tenants`, poll [`GET /tenants/status`](/api-reference/v2/endpoint/tenant-status) until `infra.scheduler_status`, `infra.graph_status`, and both entries in `infra.vectorstore_status` are `true`.
