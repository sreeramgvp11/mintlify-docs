...
<Info>
**The TypeScript SDK uses camelCase for both method names and parameter field names** (e.g., `client.context.list({ tenantId: "...", pageSize: 50 })`). The SDK serializes these to snake_case on the wire automatically  -  you write camelCase in your code, and the API receives snake_case.

The Python SDK uses snake_case throughout (e.g., `client.context.list(tenant_id="...", page_size=50)`).
</Info>
...
`client.context.*` covers every flow around content lifecycle  -  file uploads, app sources, memories, polling, fetching, listing, deletion, and graph inspection.
