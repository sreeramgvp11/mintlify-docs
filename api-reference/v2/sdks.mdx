...
The REST API uses **snake_case** for all request and response fields on the wire. The SDKs adapt this to native idioms:
...
| **TypeScript SDK** | camelCase | camelCase | method: `query()`, params: `tenantId`, `queryBy` |

<Info>
**The TypeScript SDK uses camelCase for both method names and parameter field names** (e.g., `client.source.list({ tenantId: "...", pageSize: 50 })`). The SDK serializes these to snake_case on the wire automatically  -  you write camelCase in your code, and the API receives snake_case.

The Python SDK uses snake_case throughout (e.g., `client.source.list(tenant_id="...", page_size=50)`).
</Info>
...
const status = await client.source.status({
  tenantId: "my_first_tenant",
  sourceIds: ["contract_q4", "policy_2025"],
});
```
</CodeGroup>
