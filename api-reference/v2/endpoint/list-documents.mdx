...
| <Field name="collection" type="string or null" /> | Sub-tenant scope. If omitted, the default sub-tenant is used. Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). (default=`null`) |
