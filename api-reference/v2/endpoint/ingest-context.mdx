...
| <Field name="database" type="string" required /> | Target tenant. Formerly `tenant_id`; the `tenant_id` alias is still accepted (deprecated). |
| <Field name="collection" type="string" /> | Logical partition inside the tenant. Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). (default=`""`  -  default sub-tenant) |
...
<Accordion title="App sources - index text from your workspace or personal apps">
  ...
  | <Field name="database" type="string" required /> | Target tenant. Must match the form-level `database`. Formerly `tenant_id`; the `tenant_id` alias is still accepted (deprecated). |
  | <Field name="collection" type="string" required /> | Logical partition inside the tenant. Must match the form-level `collection`. Formerly `sub_tenant_id`; the `sub_tenant_id` alias is still accepted (deprecated). |
