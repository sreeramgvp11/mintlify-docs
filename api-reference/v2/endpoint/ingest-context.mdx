...
Per-document metadata (`source_id`, `metadata`, `additional_metadata`, `title`, etc.) can be passed alongside each uploaded document to control indexing, filtering, and display. See the field reference below.
...
<Accordion title="Knowledge documents - upload PDFs, DOCX, CSV, markdown, or text">
  ...
  with open("/path/to/policy.pdf", "rb") as f1, open("/path/to/runbook.pdf", "rb") as f2:
      result = client.source.ingest(
          ...
          document_metadata=json.dumps([
              {"source_id": "policy_main", "metadata": {"department": "legal"}},
              {"source_id": "runbook_deploy", "metadata": {"department": "ops"}},
          ]),
      )
  ```
